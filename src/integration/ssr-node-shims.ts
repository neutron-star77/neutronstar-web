...
export function shironesSsrNodeShims(): Plugin {
	...
	return {
		...
		renderChunk(code) {
			...
			const lines: string[] = [];
			// workerd prerender chunks may not expose `import.meta.url` (the
			// Cloudflare adapter evaluates them from synthetic module paths), so
			// fall back to a fixed root base instead of crashing at startup.
			lines.push(
				'const __shironesSelfUrl = typeof import.meta.url === "string" ? import.meta.url : "file:///";',
			);
			...
			if (needsFilename) {
				lines.push(
					"const __filename = __shironesFileURLToPath(__shironesSelfUrl);",
				);
			}
			if (needsDirname) {
				lines.push(
					`const __dirname = __shironesDirname(${filenameDeclared ? "__filename" : needsFilename ? "__filename" : "__shironesFileURLToPath(__shironesSelfUrl)"});`,
				);
			}
			if (needsRequire) {
				lines.push(
					"const require = __shironesCreateRequire(__shironesSelfUrl);",
				);
			}
